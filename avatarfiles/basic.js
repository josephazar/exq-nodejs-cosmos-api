...
function changeLanguage() {
    ...
    // updateRecognitionLanguage(); // Update the recognition language if necessary
    window.startSession(); // Restart or initialize session with the new language
}
...
async function makePostRequest(question) {
  // alert(question)
  // 
  try {
    const url = 'https://ceersdbot.cognitiveservices.azure.com/language/:query-knowledgebases?projectName=ServiceDesk&api-version=2021-10-01&deploymentName=production';
    const postData = {
      question:question,
      top:1
    };
    ...
    const config = {
      headers: {
        ...
        'Ocp-Apim-Subscription-Key': '514f6722726d456aabe3ece2607cc5d4' 
      }
    };
...
function initializeConversation() {
    const systemMessage = {
        role: "system",
        content: "You are exquitech helper bot  which is an advanced microsoft partner that does everything related to microsoft services from AI to business central to power platform.  You share typical uses cases for the various products and can you offer recommendations for products in response to customore needs."   
    };
...
async function fetchOpenAIResponse(userMessage) {
  ...
  const response = await fetch('https://exquitech-openai-2.openai.azure.com/openai/deployments/ex-turbo-16k/chat/completions?api-version=2023-12-01-preview', {
    method: 'POST',
    headers: {
  ...
  // alert(JSON.stringify(conversationHistory))
  console.log(JSON.stringify(conversationHistory))
  console.log(`api-key${OPENAI_API_KEY }`)
  console.log(`${JSON.stringify(conversationHistory[conversationHistory.length - 1].content)}`)
  // Handle response
  if (!response.ok) {
...
function scrollToBottom(elementId) {
  ...
  function speakAndFetchResponse() {
    ...
    //   });
      ...
      makePostRequest(userQuery).then((response)=>{
        // alert(response.answers[0]?.answer)
        stopProcessingIndicator();
        streamResponseText('openAIResponse', response.answers[0]?.answer, 50);
        
        console.log("Received response from QNA :", response.answers?.answer);
        if (avatarSynthesizer && typeof avatarSynthesizer.speak === 'function') {
          // alert("function")

          if(language=="Arabic"){
            translatetoarabic(response.answers[0].answer).then((s)=>{
              console.log("translate to arabic"+s)
              avatarSynthesizer.speak(s);
              console.log("Passed response to avatar synthesizer.");
            })
            ...
            avatarSynthesizer.speak(response.answers[0].answer);
          }
...
window.startSession = () => {
    ...
    if(language=="Arabic"){
      speechSynthesisConfig.speechSynthesisLanguage = "ar-LB";
      speechSynthesisConfig.speechSynthesisVoiceName = config.TTSConfiguration.arabicVoice;
    }
    else{
      // alert("voice is english")
      speechSynthesisConfig.speechSynthesisLanguage = "en-US-JennyNeural";
      speechSynthesisConfig.speechSynthesisVoiceName = config.TTSConfiguration.englishVoice;
    }
...
// setTimeout(function() {
//       // alert("calling")
//       // Call the startSession function after 10 seconds
//       window.startSession();
//     }, 2000);
...
async function translatetoarabic(userMessage) {

// or use the appropriate model you have access to
messages= [
    {
      role: "system",
      content: "You are a helpful assistant that translate text to arabic."
    },
    {
      role: "user",
      content: `${userMessage} . \n output only the translated text.`
    }
    // You can add more messages to simulate an ongoing conversation
  ]
